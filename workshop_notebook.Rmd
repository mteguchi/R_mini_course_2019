...
```{r}
growth_data_clean %>% filter(Turtle_ID == "3030") -> turtle3030_2
turtle3030_2
```
...
turtle3030_2 <- growth_data_clean[growth_data_clean$Turtle_ID == "3030",]
turtle3030
```
...
ggplot(data = turtle3030) + 
  geom_point(aes(x = Date, y = SCL), na.rm = T) + 
...
tmp <- filter(turtle3030_2, SCL != "NA")

turtle3030 <- filter(turtle3030_2, !is.na(SCL))
ggplot(data = turtle3030) + 
...
growth_data_clean %>% 
  subset(Turtle_ID == "3030") %>%
  mutate(Date = as.Date(Date_Caught, 
                        format = "%m/%d/%Y")) -> turtle3030
...
n_caught1 <- filter(growth_data_clean, SCL>60) %>% 
  count(., Turtle_ID)
```
...
growth_data_clean %>% 
  mutate(Size.Class = if_else(SCL > 90, 3, 
...
```{r}
lm.data %>% filter(CooksD2 <= 0.1) -> lm.data1
fit.lm3 <- lm(lnWeight ~ SCL, data = lm.data1)

lm.data1$yhat <- predict(fit.lm3)

p3 <- ggplot(data = lm.data1) + 
  geom_point(aes(x = SCL, 
                 y = lnWeight, 
                 color = CooksD2)) + 
  geom_line(aes(x = SCL, y = yhat),
...
data.0$Month <- as.factor(as.numeric(format(as.Date(data.0$Date), '%m')))
